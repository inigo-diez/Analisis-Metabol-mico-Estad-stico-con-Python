...
Este proyecto implementa un pipeline completo de análisis metabolómico aplicado a datos de cromatografía de gases acoplada a espectrometría de masas (GC-MS). Los datos proceden de 17 experimentos los cuales se realizaron en condiciones diferenter para evaluar la eficiencia de cada uno. Los datos han sido generados a partir de un conjunto de muestras anonimizado y generadas artificialmente ya que no se permite el uso de datos personales de participantes en este proyecto. Se detectaron aproximadamente 1.230 picos cromatográficos y tras aplicar un filtrado por calidad de identificación espectral (Match Factor ≥ 700), se seleccionaron 279 metabolitos para el análisis, que posteriormente se anotaron frente a una base de datos de compuestos asociados a condiciones de salud mental.

El objetivo del proyecto es explorar estos perfiles metabolómicos para identificar metabolitos candidatos con patrones diferenciales entre grupos de muestras, además de escoger las condiciones experimentales óptimas para desarrollar el experimento teniendo en cuenta la cantidad de metabolitos de interés obtenidos. Para ello se combinan técnicas de análisis exploratorio y métodos de aprendizaje automático. El pipeline incluye reducción de dimensionalidad mediante PCA, métodos de clustering no supervisado (clustering jerárquico y K-means), pruebas estadísticas con corrección FDR y una exploración mediante modelos de clasificación supervisada (Random Forest y XGBoost) junto con análisis de importancia de variables mediante SHAP.
...
El notebook [00 – Pipeline completo](00_proyecto_completo.ipynb) integra todos los análisis anteriores en un único documento con interpretaciones de cada visualización.
